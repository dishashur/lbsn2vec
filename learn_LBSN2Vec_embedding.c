...
//undone ----------- called only by learn_an_edge, which is not called by anyone
...
//undone ---- commented out ----- not used anywhere
...
//                             printf("mobility \n");
                            user_pr = mxGetCell(user_checkins, word-1); //Pointer to the ith cell mxArray
...
/*learn_LBSN2Vec_embedding(walks',user_checkins, user_checkins_counter,...
    embs_ini', learning_rate, K_neg,...
    neg_sam_table_social, win_size, neg_sam_table_mobility_norm,...
    num_epoch,num_threads,mobility_ratio); -----> this is what I have sent */
