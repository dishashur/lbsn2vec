...
class DataSet(object):
    ...
    def next_batch(self, embeddings, batch_size=16, num_neg_samples=1, pair_radio=0.9, sparse_input=True):
        ...
        while 1:
            ...
            yield (dict([('input_{}'.format(i), batch_e[i]) for i in range(4)]),
                    dict([('decode_{}'.format(i), batch_e[i]) for i in range(4)]+[('classify_layer', labels)]))

def embedding_lookup(embeddings, index, sparse_input=True):
    if sparse_input:
        return [embeddings[i][index[:, i], :].todense()  for i in range(4)]
    else:
        return [embeddings[i][index[:, i], :]  for i in range(4)]
...
def generate_H(edge, nums_type):
    ...
    #print(max(edge[:,0]),',',nums_type[0])
    #print(max(edge[:,1]),',',nums_type[1])
    #print(max(edge[:,2]),',',nums_type[2])
    #print(max(edge[:,3]),',',nums_type[3])
    H = [csr_matrix((np.ones(nums_examples), (edge[:, i], range(nums_examples))), shape=(nums_type[i], nums_examples)) for i in range(4)]
    return H
...
def generate_embeddings(edge, nums_type, H=None):
    ...
    embeddings = [H[i].dot(s_vstack([H[j] for j in range(4) if j != i]).T).astype('float') for i in range(4)]
    ### 0-1 scaling
    for i in range(4):
